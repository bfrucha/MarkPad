...
//  (c) Eric Lecolinet 2016/17 - http://www.telecom-paristech.fr/~elc
//
...
int Socket::setAddress(struct sockaddr_in& addr, const string& host, int port) {
  ...
  if (host.empty() || !(hent = ::gethostbyname(host.c_str()))) return -1; // host not found
    ...
    addr.sin_family = AF_INET;
    addr.sin_port = htons(port);
    // note: data may be misaligned but the result is correct because memcpy() is used
...
ssize_t SocketBuffer::writeLine(const string& str) {
  ...
  if (len <= _outSize) {
    ...
    if (_outSep >= 0) buffer[len-1] = char(_outSep);     // add separator
    else {
      buffer[len-2] = '\r';
      buffer[len-1] = '\n';
    }
